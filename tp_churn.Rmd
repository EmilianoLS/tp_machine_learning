...
```{r}
# Armo una matriz de correlacion
Matriz_correlacion <- round(cor(data,  use = "na.or.complete"), digits = 5)

# filtro por el umbral de correlacion
indice_col_0.9 <- c(findCorrelation(Matriz_correlacion, 0.9))
indice_col_0.8 <- c(findCorrelation(Matriz_correlacion, 0.8))
indice_col_0.7 <- c(findCorrelation(Matriz_correlacion, 0.7))
# identifico las variables con correlacion
Columnas_Cor.9 <-names(data[,indice_col_0.9])
Columnas_Cor.8 <-names(data[,indice_col_0.8])
Columnas_Cor.7 <-names(data[,indice_col_0.7])

# identifico la varianza para ir explorando las variables de mi data.frame
nzv <- as.data.frame(nearZeroVar(data, saveMetrics = TRUE))

Zer_var <- row.names(nzv[nzv$zeroVar == TRUE,])

Nzv_features <- row.names(nzv[nzv$nzv == TRUE,]) # Me quedo con las que tienen varianza cercana a cero

```

Vemos que hay 153 variables con una correlacion mas elevada de 0.9, si nos fijamos en una correlacion mayor a 0.8 se añaden 38 variables a la cuenta para un total de 191 y para una correlacion mayor a 0.7 se añaden 30 variables mas, para un total de 221 variables es decir un 0.71% de las variables del data set, esto nos podria traer problemas de multicolinealiad con nuestro modelo y afectar su performance, por lo que hay que trabajar con las variables que mejor expliquen nuestro modelo. 

En este sentido tambien exploramos la varianza en los datos de cada columna, encontramos que ninguno tiene varianza 0 y hay 257 datos que si tienen una varianza cercana a 0, que podemos interpretar esto en que tengan poca incidencia en la funcion a determinar por el modelo o por el contrario, que tengan alto grado de impacto en la funcion y capacidad de prediccion del modelo. Importante para entender el data set de entrada.
...
Lo que haremos a continuación es correr un modelo de clasificación con lasso por dos motivos:

* Mejorar el modelo benchmark intentando restringir el modelo en cuanto a las variables 
* Hacer selección de variables 

Se van a correr modelos para distintos valores de lambda, validándolos mediante cross validation, nos quedaremos con el mejor y lo evaluaremos sobre el conjunto de *validation*. Si se obtiene una mejora respecto del benchmark entonces podemos considerarlo una mejora, sin embargo, apuntamos a reducir el número de variables para mantener las más relevantes
...
#train_data$TARGET <- as.logical(if_else(train_data$TARGET == 'churn',1,0))
#validation_data$TARGET <- as.logical(if_else(validation_data$TARGET == 'churn',1,0))
...
############################################## MODELO DE REG LOG REGULARIZADO ###################################################
# Creamos una secuencia de valores para lambda

grid.l =exp(seq(1 , -20 , length = 50))
grid.l
...
saveRDS(cv.out, "lasso_opt_model.rds")
...
saveRDS(rf_default, "rf_basic_model.rds") # Guardo el modelo
...
tunegrid <- expand.grid(.mtry = c(156))
modellist <- list()
...
for (ntree in c(1000,1500,2000,2500)){
  ...
  modellist[[key]] <- c(max(rf_optimized$results$ROC))
}

# Corte el modelo antes porque iba a tardar mucho y no vale la pena porque no va a ser el modelo final este
# La mejor de las MTRY fue de 118 variables asi que ahora vamos a hacer lo siguiente:

# Probar el random forest con distintas cantidades de árboles, manteniendo MTRY = 156 (optimo anterior) 
# Una vez seleccionado la mejor cantidad de arboles, vuelvo a probar el modelo pero evaluando MTRY entre 156 y 118

saveRDS(rf_default, "rf_deep_model.rds") # Guardo el modelo

```


```{r}
# Selecciono de una grilla de valores de mtry el mejor que este entre 118 y 156 manteniendo la mejor profundidad de 2500 arboles
...
tunegrid <- expand.grid(.mtry = seq(118,156, by = 10)) 

rf_gridsearch_2 <- train(TARGET ~ ., 
                       data = train_data,
                       method = 'rf',
                       metric = 'ROC',
                       tuneGrid = tunegrid,
...
saveRDS(rf_gridsearch_2, "rf_final.rds") # Guardo el modelo

# Guardo el mejor AUC de Train

rf_optimized_final_train <- max(rf_gridsearch_2$results$ROC)

# Evaluamos en Test

pred_test <- predict(rf_gridsearch_2,validation_data, type = 'prob')
pred_test$pred <- factor(if_else(pred_test$churn >= .5, 'churn','no_churn'))
pred_test$obs <- validation_data$TARGET

test_results_rf_final <- twoClassSummary(pred_test, lev = levels(pred_test$obs))
test_results_rf_final_roc <- as.numeric(test_results_rf_final[1])

# Muestro resultados

modelo <- c('Random forest optimizado')

data.frame(modelo, rf_optimized_final_train,test_results_rf_final_roc)

# Guardo las variables mas importantes

rf_important_features <- varImp(rf_gridsearch_2, scale = TRUE)
```

```{r}
# Comparamos el modelo Lasso y todos los Random forest

lasso <- readRDS("lasso_opt_model.rds")
rf_basic <- readRDS("rf_basic_model.rds")
rf_optimized <- readRDS('rf_deep_model.rds')
rf_final <- readRDS('rf_final.rds')

modelos_feature_selection <- c('Lasso','Random Forest Basico','Random Forest 1 optimizacion','Random Forest Final')

################################################ LASSO #############################################################

pred_train_lasso = predict(lasso, s = lasso$lambda.min , newx = x_train, type = 'response')

# Predicciones en Test

pred_test_lasso = predict(lasso, s = lasso$lambda.min , newx = x_test, type = 'response')

# Calculo la curva ROC en Train

pred2_train <- prediction(pred_train_lasso, y_train)
lasso_train_result <- performance(pred2_train,"tpr","fpr")

# Calculo la curva ROC en Test

pred2_test <- prediction(pred_test_lasso, y_test)
lasso_test_result <- performance(pred2_test,"tpr","fpr")

# Calculamos el AUC en Train

lasso_train_result_roc <- performance(pred2_train, measure = "auc")
lasso_train_result_roc <- lasso_train_result_roc@y.values[[1]] 

# Calculamos el AUC en Test

lasso_test_result_roc <- performance(pred2_test, measure = "auc")
lasso_test_result_roc <- lasso_test_result_roc@y.values[[1]] 
...
rf_basic_train_result <- max(rf_basic$results$ROC)

# Evaluamos en Test

pred <- predict(rf_basic,validation_data, type = 'prob')
pred$pred <- factor(if_else(pred$churn >= .5, 'churn','no_churn'))
pred$obs <- validation_data$TARGET

rf_basic_test_result <- twoClassSummary(pred, lev = levels(pred$obs))
rf_basic_test_result_roc <- as.numeric(rf_basic_test_result[1])

################################################ 1 OPTIMIZACION #####################################################

# Evaluamos en Train

rf_cv_train_result <- max(rf_optimized$results$ROC)
...
rf_cv_test_result <- twoClassSummary(pred, lev = levels(pred$obs))
rf_cv_test_result_roc <- as.numeric(rf_cv_test_result[1])

################################################ FINAL #############################################################

# Evaluamos en Train

rf_final_train_result <- max(rf_final$results$ROC)

# Evaluamos en Test

pred <- predict(rf_final,validation_data, type = 'prob')
pred$pred <- factor(if_else(pred$churn >= .5, 'churn','no_churn'))
pred$obs <- validation_data$TARGET

rf_final_test_result <- twoClassSummary(pred, lev = levels(pred$obs))
rf_final_test_result_roc <- as.numeric(rf_final_test_result[1])

####################################################################################################################
# Muestro todos los resultados

train_results <- c(lasso_train_result_roc, rf_basic_train_result, rf_cv_train_result, rf_final_train_result)
test_results <- c(lasso_test_result_roc, rf_basic_test_result_roc, rf_cv_test_result_roc, rf_final_test_result_roc)
data.frame(modelos_feature_selection, Train_Results = train_results, Test_Results = test_results)
```

Obtuvimos una serie de modelos con buenos resultados, ahora veamos la relevancia de las variables para cada uno:

```{r}
# Grafico las variables mas importantes

# Lasso
coef_matrix <- as.matrix(coef(lasso, lasso$lambda.min))
important_features <- data.frame(importance = as.matrix(coef_matrix[coef_matrix != 0,]))
important_features$feature <- row.names(important_features)
row.names(important_features) <- seq(1:nrow(important_features))

# Graficamos

par(mfrow = c(1,4))

ggplot(important_features, aes(x=reorder(feature,importance), y=importance)) + geom_bar(stat = "identity") + coord_flip()
plot(varImp(rf_basic), top = 30, main = 'Random Forest Basico')
plot(varImp(rf_optimized), top = 30, main = 'Random Forest Optimizado')
plot(varImp(rf_final), top = 30, main = 'Random Forest Final')


```

Por último nos fijamos qué variables importantes coinciden:

```{r}

# VARIABLES LASSO

lasso_features <- important_features$feature

# VARIABLES RANDOM FOREST

rf_features <- varImp(rf_final)
rf_features <- data.frame(rf_features$importance)
rf_features$feature <- row.names(rf_features)
row.names(rf_features) <- seq(1:nrow(rf_features))
rf_features <- rf_features[order(-rf_features$Overall),]
rf_features <- rf_features[rf_features$Overall >= 1,]
rf_features <- rf_features$feature

# Variables coincidentes 

intersect(lasso_features, rf_features)

```
Ya tenemos nuestro set de variables más relevantes, lo siguiente es revisarlas, ver las que coinciden y de las que no, si podemos eliminar algunas más en base a criterios de correlación y variabilidad, para finalmente ensamblar todo en un último modelo final.

No corremos ningún modelo solamente con estas variables porque en teoría, no mejoraría la performance.

#### Refinamiento de las variables más importantes

```{r}
# Selecciono todas las variables de ambos modelos

most_important_features <- unique(c(lasso_features, rf_features))
most_important_features <- sub('TRUE','', most_important_features) # Elimino la palabra TRUE de las varables logicas
most_important_features <- sub('FALSE','', most_important_features) # Elimino la palabra TRUE de las varables logicas
most_important_features <- most_important_features[-1]              # Elimino la variable 'Intercept'

# Calculo cualquier posible correlación entre las variables

corr_mat <- cor(train_data[,most_important_features])

# Cuento features muy correlacionados

highCorr <- sum(abs(corr_mat[upper.tri(corr_mat)]) > .999)
highCorr # Hay 11 features con correlacion perfecta!! Podemos eliminar uno de los dos

# Usamos la funcion findCorrelation para eliminar una de las dos variables mas correlacionadas

highlyCorDescr <- findCorrelation(corr_mat, cutoff = .7)
length(highlyCorDescr) # Hay 55 features con correlación de mas de 70%, eliminamos una de las dos
train_data_filtered <- train_data[, -highlyCorDescr]

# Filtramos variables con varianza cero
# Se corrio y se comprobo que no había mejoras sino que bajaba la performance
#nzv <- nearZeroVar(train_data_filtered, saveMetrics = TRUE)
#not_zv_features <- row.names(nzv[nzv$zeroVar == FALSE,]) # Me quedo con las que no tienen varianza cero
#train_data_filtered <- train_data_filtered[,not_zv_features]
#data[, nzv]
```

## Modelo final: XGBOST sin optimizar
...
xgb_fit <- train(TARGET ~., data = train_data_filtered, 
                method = "xgbTree",
                trControl=trctrl,
                metric = 'ROC',
                preProcess = c('center','scale'))
...
saveRDS(xgb_fit, "xbg_basic.rds") # Guardo el modelo
...
xgb_fit <- readRDS("xbg_basic.rds")

xgb_train_result <- max(xgb_fit$results$ROC)
xgb_train_result

# Evaluamos en Test

validation_data_filtered <- validation_data[,-highlyCorDescr] # Filtramos las variables relevantes
#validation_data_filtered <- validation_data[,not_zv_features]

pred <- predict(xgb_fit,validation_data_filtered, type = 'prob')
pred$pred <- factor(if_else(pred$churn >= .5, 'churn','no_churn'))
pred$obs <- validation_data_filtered$TARGET

xgb_test_result <- twoClassSummary(pred, lev = levels(pred$obs))
xgb_test_result_roc <- as.numeric(xgb_test_result[1])
xgb_test_result_roc

# Graficamos las curvas ROC

pred <- predict(xgb_fit,validation_data_filtered, type = 'prob')[,2]
pred2 <- prediction(pred, validation_data_filtered$TARGET)
perf <- performance(pred2,"tpr","fpr")

plot(perf, main="Curva ROC Testing - XGBOOST", colorize=T)

```
### Hacemos un grid search para optimizar el modelo final
...
xgb_fit_cv <- train(TARGET ~., data = train_data_filtered, 
                method = "xgbTree",
                trControl=trctrl,
                metric = 'ROC',
                preProcess = c('center','scale'),
...
```{r}
saveRDS(xgb_fit_cv, "xbg_cv.rds") # Guardo el modelo
```
