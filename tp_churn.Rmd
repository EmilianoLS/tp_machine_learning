...
plot2 <- data %>% ggplot(aes(x = var36, fill = TARGET)) + geom_histogram(color="#e9ecef", alpha=0.4, position = 'identity', bins = 50) + ggtitle('Distribución de la var38 según el target')
...
grid.arrange(plot1,plot2,plot3,ncols = 1)
```
...
Sin embargo, cuando normalizamos notamos que tiene demasiadas variables muy cercanas a 0 con muy poca variabilidad, incluso con una transformación logarítmica, muchas variables tienen el 75% de sus datos en un valor de 0.
