...
```{r}
# Normalizo los datos
train_data_norm <- train_data_norm[,-nzv]
validation_data_norm <- validation_data_norm[,-nzv]
...
```{r}
# ROC y AUC
pred_t = predict(rpartFit_norm, newdata = validation_data_norm ,type='prob')[,2]
pred2 <- prediction(pred_t, validation_data_norm$TARGET)
perf <- performance(pred2,"tpr","fpr")
# Graficamos la curva ROC
plot(perf, main="Curva ROC", colorize=T)
...
```{r}
# Predecimos sobre datos de validation_data_norm
pred <- predict(rpartFit_norm,validation_data_norm, type = 'prob')
pred$pred <- factor(if_else(pred$churn >= .5, 'churn','no_churn'))
pred$obs <- validation_data_norm$TARGET
