...
plot (cv.out, main = 'AUC en función de lambda')
...
matriz_var <- as.matrix(coef(cv.out))
temp <- as.matrix(matriz_var[matriz_var > 0,])
variables_importantes_lasso <- row.names(temp) # Obtengo las variables con coeficientes distintos de cero

# Predicciones en Train

pred_tr_lasso = predict(cv.out, s = bestlam , newx = x_train, type = 'response')

# Predicciones en Test

pred_test_lasso = predict(lasso.sin.reg, s = bestlam , newx = x_test, type = 'response')

# Calculo la curva ROC en Train


pred2_lasso <- prediction(pred_tr_lasso, y_train)
perf_tr_lasso <- performance(pred2_lasso,"tpr","fpr")

plot(perf_tr_lasso, main="Curva ROC", colorize=T)

auc_lasso_train <- performance(pred2_lasso, measure = "auc")
auc_lasso_train@y.values[[1]] 

# Calculo la curva ROC en Test


pred2_test_lasso <- prediction(pred_test_lasso, y_test)
perf_test_lasso <- performance(pred2_test_lasso,"tpr","fpr")

plot(perf_test_lasso, main="Curva ROC", colorize=T)

auc_lasso_test <- performance(pred2_test_lasso, measure = "auc")
auc_lasso_test@y.values[[1]]
